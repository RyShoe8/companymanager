...
export default function ClientCalendarView({
  ...
  const renderClientDayView = (day: Date, paddingClass = 'px-0 py-2 md:p-6') => {
      ...
      <div className={`${paddingClass} space-y-3`}>
        {dayRows.map((row) => {
          ...
          return (
            <div
              key={clientId}
              className="p-1 md:p-6 rounded-lg border-2 border-border transition-all duration-300 hover:-translate-y-1 hover:shadow-xl relative overflow-hidden"
              style={{
                backgroundColor: `${displayColor}F0`,
                borderColor: displayColor,
              }}
    ...
    if (isMobile) {
      return (
        <WeeklyMobileDayPager startDate={startDate} weekKey={startDate.toISOString()}>
          {(day) => renderClientDayView(day, 'px-0 py-2')}
