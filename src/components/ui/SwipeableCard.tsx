...
export default function SwipeableCard({ children, leftActions = [], rightActions = [], className = '', threshold = 80 }: SwipeableCardProps) {
  ...
  return (
    <div ref={containerRef} className={`relative overflow-hidden ${className}`}>
      ...
      <div className={`relative bg-white dark:bg-gray-800 transition-transform ${isDragging ? '' : 'duration-200'}`} style={{ transform: `translateX(${offset}px)` }} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>{children}</div>
    </div>
  );
}
