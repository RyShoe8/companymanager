...
describe('admin connection diagnostics', () => {
  it('accepts named recovery attempts and rejects arbitrary recovery retries', async () => {
    ...
    expect((await POST(request({ confirm: true, kind: 'chat-recovery-2' }))).status).toBe(200);
    expect(mocks.run).toHaveBeenCalledWith('admin', 'chat-recovery-2');
    expect((await POST(request({ confirm: true, kind: 'chat-recovery-3' }))).status).toBe(400);
  ...
  it('permits only the named new one-time attempt, not arbitrary retry IDs', async () => {
    ...
    expect((await POST(request({ confirm: true, kind: 'chat-recheck-2' }))).status).toBe(400);
