...
export async function GET() {
  ...
  try { return aiResponse(await readExecutionProbe()); } catch (error) { return aiError(error); }
}
export async function POST(request: Request) {
  const auth = await requirePlatformAdmin(); if (auth.error) return auth.error;
  try {
    if (request.headers.get('origin') !== new URL(request.url).origin) throw new AiHttpError(403, 'Invalid request origin.');
    z.object({ confirm: z.literal(true) }).strict().parse(await readAiBody(request));
    return aiResponse(await runExecutionProbe(String(auth.user._id)));
